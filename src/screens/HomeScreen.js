  ...
  return (
    <View style={styles.timeDetail}>
      ...
      <Text style={{fontFamily: 'OpenSans-SemiBold'}}>
        {checkIn == '' || checkIn == '00:00:00' ? '--:--' : checkIn}
      </Text>
    </View>
  );
}
...
function EndTime({checkOut}) {
  return (
    <View style={styles.timeDetail}>
      <Text
        style={{
          color: CONSTANT.STYLE.COLOR.MWR_RED,
          fontSize: CONSTANT.STYLE.SIZE.SUB_TITLE,
          fontFamily: 'OpenSans-SemiBold',
        }}>
        Waktu Akhir
      ...
      <Text style={{fontFamily: 'OpenSans-SemiBold'}}>
        {checkOut == '' || checkOut == '00:00:00' ? '--:--' : checkOut}
      </Text>
    </View>
  );
}
...
function IconButton({image_url, name, navigation, getCurrentHours}) {
  ...
  return (
    <View style={styles.IconButton}>
      <TouchableOpacity style={{alignItems: 'center'}} onPress={onButtonPress}>
        <Image
          source={image_url}
          style={{
...
export default function Home({navigation}) {
  ...
  let todayAttendance = useSelector(
    state => state.attendanceReducers.todayAttendance,
  );

  return (
    <Layout>
      <View
        style={{flex: 1, marginTop: vs(15), justifyContent: 'space-around'}}>
        <View style={styles.logoContainer}>
          <Image
            source={CONSTANT.IMAGE_URL.LOGO}
            style={{
              width: s(
                Image.resolveAssetSource(CONSTANT.IMAGE_URL.LOGO).width * 0.9,
              ),
              height: vs(
                Image.resolveAssetSource(CONSTANT.IMAGE_URL.LOGO).height * 0.13,
              ),
              resizeMode: 'contain',
            }}
          />
      ...
      <View style={{flex: 3, justifyContent: 'center'}}>
        ...
        <View style={{alignItems: 'center', marginBottom: vs(25)}}>
          <ImageBackground
            source={CONSTANT.IMAGE_URL.CLOCK}
            style={{
              padding: vs(15),
              width: vs(
                Image.resolveAssetSource(CONSTANT.IMAGE_URL.CLOCK).width * 0.2,
              ),
              height: vs(
                Image.resolveAssetSource(CONSTANT.IMAGE_URL.CLOCK).height * 0.2,
              ),
              alignItems: 'center',
              resizeMode: 'contains',
