      ...
      <View>
        <TouchableOpacity
          onPress={() => {
            Alert.alert(
              '',
              'Please check in or check out first',
              [
                {
                  text: 'Ok',
                  onPress: () => {},
        ...
        <RowComponent attendanceData={attendanceData} navigation={navigation} />
      </View>
    );
  } else {
    return (
      <View style={styles.rowContainer}>
        <TouchableOpacity
          onPress={() => {
            if (
              attendanceData.latCheckIn !== '' ||
              attendanceData.latCheckOut !== ''
            ) {
  ...
  return (
    <View style={styles.timeContainer}>
      <Image
        ...
        style={{
          width: s(
            Image.resolveAssetSource(CONSTANT.IMAGE_URL.MARKER_START_TIME)
              .width * 0.07,
          ),
          height: vs(
            Image.resolveAssetSource(CONSTANT.IMAGE_URL.MARKER_START_TIME)
  ...
  return (
    <View style={styles.timeContainer}>
      <Image
        ...
        style={{
          width: s(
            Image.resolveAssetSource(CONSTANT.IMAGE_URL.MARKER_END_TIME).width *
              0.07,
          ),
          height: vs(
            Image.resolveAssetSource(CONSTANT.IMAGE_URL.MARKER_END_TIME)
              .height * 0.07,
          ),
          resizeMode: 'contain',
        }}
...
export default function History({navigation}) {
  ...
  let attendanceHistory = useSelector(
    state => state.attendanceReducers.attendanceHistory,
  );
